...
content = content.replace(
  "w-full lg:w-1/2 flex items-center",
...
if (rightSideStart !== -1) {
  const endDiv = content.indexOf(
    "</div>",
    content.indexOf("</Image>", rightSideStart) ||
      content.indexOf("/>", rightSideStart),
  );
  ...
  const regex =
    /\{\/\* Right side: Splitted Image \*\/\}[\s\S]*?<\/div>(\r?\n\s*<\/div>\r?\n\s*\);\r?\n\})/;
