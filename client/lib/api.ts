...
/**
 * Thrown by the client quote helper when the admin pricing endpoint is
 * unavailable. Mirrors the server-side PricingUnavailableError.
 */
export class PricingUnavailableError extends Error {
  constructor(message = "Pricing unavailable — contact dispatch") {
    super(message);
...
export const api = {
  ...
  quotes: {
    /**
     * Get an authoritative fare quote for a pricing preview. The server
     * proxies to the admin pricing endpoint (ADMIN_API_URL stays server-side).
     * At booking time the server re-quotes again — never trust a client price.
     */
    async get(input: FareQuoteInput): Promise<FareQuote> {
      try {
        const res = await apiRequest("POST", "/api/quote", input);
        return await res.json();
      } catch (e: any) {
        const msg = e?.message || "Pricing unavailable — contact dispatch";
