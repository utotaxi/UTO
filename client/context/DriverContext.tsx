...
function mapRidePayload(ride: any): RideRequest {
  const discountAmount = Math.max(
    0,
    Number(ride.discountAmount ?? ride.discount_amount ?? 0),
  );
...
export function DriverProvider({ children }: { children: ReactNode }) {
  ...
  const [driverProfile, setDriverProfileState] = useState<DriverProfile | null>(
    null,
  );
  ...
  const [activeRideRequest, setActiveRideRequest] =
  ...
  const [completedRidePayment, setCompletedRidePayment] =
    useState<CompletedRidePayment | null>(null);
  const [paidWaitingStartedAt, setPaidWaitingStartedAt] = useState<
    string | null
  >(null);
  const [waitingChargePerMin, setWaitingChargePerMin] = useState(0.5); // default £0.50/min
  ...
  const handleRidePayload = useCallback((ride: any) => {
    ...
    if (ride.scheduledPreAccepted) {
      if (
        ...
        activeRideRequestRef.current.id !== ride.id
      ) {
        console.warn(
          "⚠️ Scheduled ride went live but driver is busy with another ride:",
          activeRideRequestRef.current.id,
        );
        return;
      }
  ...
  const fetchPendingDispatch = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${getApiUrl()}/api/drivers/${driverId}/pending-dispatch`,
      );
      if (!res.ok) return;
      const data = await res.json();
      if (data?.ride?.id) {
        console.log(
          "📲 Restored pending ride dispatch from server:",
          data.ride.id,
        );
  ...
  useEffect(() => {
    ...
    try {
      cleanup = onNewRide((ride: any) => {
        console.log(
          "🚕 Driver received new ride request via socket payload:",
  ...
  useEffect(() => {
    ...
    startBackgroundLocationTracking(driverId).catch((err) =>
      console.warn(
        "⚠️ Failed to ensure background location while online:",
        err,
      ),
    );
    ...
    try {
      cleanupExpired = onRideExpired((data) => {
        ...
        if (currentRide && currentRide.id === data.rideId) {
          console.log(
            `⏱️ Ride ${data.rideId} expired because to no response. Clearing screen.`,
          );
          setActiveRideRequest(null);
          setRideState("none");
        }
        ...
        if (
          ...
          if (matchesActive) {
            ...
            if (update.status === "cancelled_no_show") {
              ...
              console.log(
                "ℹ️ Server confirmed no-show for ride",
                update.rideId,
                "— refreshing data",
              );
              setActiveRideRequest(null);
              setActiveRide(null);
              saveActiveRide(null).catch((err) =>
                console.warn("⚠️ Failed to clear active ride:", err),
              );
              setRideState("none");
              ...
              setTimeout(() => {
                refreshData().catch((err: any) =>
                  console.warn("⚠️ Post rider-cancel refreshData failed:", err),
                );
              }, 1500);
            }
            ...
            console.log(
              "ℹ️ No-show server confirmation received but ride already cleared locally",
            );
            setTimeout(() => {
              refreshData().catch((err: any) =>
                console.warn("⚠️ Post no-show refreshData failed:", err),
  ...
  const loadStoredData = async () => {
    try {
      const [storedProfile, storedTrips, storedOnline, storedActiveRide] =
        await Promise.all([
      ...
      if (storedTrips) {
        ...
        if (parsed.length > 0) {
          ...
          console.log(
            "✅ Loaded",
            parsed.length,
            "cached trips from AsyncStorage",
          );
        }
      }
      ...
      if (storedActiveRide) {
        try {
          ...
          console.warn(
            "⚠️ Failed to parse active ride from AsyncStorage:",
            err,
          );
        }
      }
  ...
  const refreshData = async () => {
    ...
    try {
      ...
      const profile: DriverProfile = {
        id: driver.id,
        vehicleType:
          (driver.vehicleType as DriverProfile["vehicleType"]) || "saloon",
      ...
      console.log(
        "✅ Driver profile synced from Supabase:",
        profile.vehicleMake,
        ...
        if (rides && rides.length > 0) {
          const completedRides = rides.filter(
            (r: any) =>
              r.status === "completed" ||
              (r.status === "cancelled" &&
                (r.paymentStatus === "no_show_card_charged" ||
                  r.paymentStatus === "no_show_wallet_charged" ||
                  r.paymentStatus === "no_show_fee")) ||
          ...
          const serverTrips: Trip[] = completedRides.map((r: any) => {
            ...
            return {
              ...
              completedAt: normalizeBackendTimestamp(
                r.completedAt || r.requestedAt || new Date().toISOString(),
              ),
          ...
          await AsyncStorage.setItem(
            TRIP_HISTORY_KEY,
            JSON.stringify(serverTrips),
          );
          console.log(
            "✅ Driver trips synced from Supabase count:",
            serverTrips.length,
          );
        }
      } catch (tripErr) {
      ...
      try {
        ...
        try {
          ...
          if (rides && rides.length > 0) {
            ...
            cancelledRides.forEach((ride: any) => {
              ...
              const paymentStatus = String(
                ride.paymentStatus || ride.payment_status || "",
              ).toLowerCase();
              const cancelledBy = String(
                ride.cancelled_by || ride.cancelledBy || "",
              ).toLowerCase();
              const storedFee = Number(
                ride.cancellation_fee ?? ride.cancellationFee ?? 0,
              );
              ...
              const isRiderCancelCredit =
                cancelledBy === "rider" ||
                riderCancelPaymentStatuses.has(paymentStatus);

              if (storedFee > 0) {
                feeAmount = storedFee;
              } else if (
                isRiderCancelCredit &&
                paymentStatus.includes("cancellation")
              ) {
                // Payable fare (with or without coupon) — same as Total Fare charged to rider
                feeAmount = getDiscountedFare(
                ...
                feeAmount = getDriverCancelPenalty(
                  ride.estimatedPrice || 0,
                  ride.discountAmount || 0,
                );
              }
              ...
              if (feeAmount > 0) {
                ...
                const driverPenaltyLabel = formatLiveRideCancellationPenalty(
                  ride.id,
                );
          ...
          console.warn(
            "⚠️ Could not fetch cancelled rides for deductions:",
            ridesErr,
          );
        }
        ...
        console.warn(
          "⚠️ Could not sync deductions from Supabase:",
          deductionErr,
        );
      }
  ...
  const fetchActiveRides = async () => {
    ...
    try {
      const baseUrl = getApiUrl();
      const res = await fetch(
        `${baseUrl}/api/drivers/${driverProfile.id}/active-rides`,
      );
      if (!res.ok) return;
      const data = await res.json();
    ...
    if (activeRide && !activeRideRequest && !isLoading) {
      ...
      const restoreActiveRideRequest = async () => {
        try {
          ...
          if (!res.ok) {
            console.warn(
              `⚠️ Could not fetch ride details for ${activeRide.id}, server returned ${res.status}`,
            );
            return;
          }
          const data = await res.json();
          ...
          const rideRequest: RideRequest = {
            ...
            pickupAddress:
              ride.pickupAddress || activeRide.pickupAddress || "Pickup",
            dropoffAddress:
              ride.dropoffAddress || activeRide.dropoffAddress || "Dropoff",
            ...
            dropoffLongitude:
              ride.dropoffLongitude || ride.dropoff_longitude || 0,
            estimatedFare: payableFare,
            distanceMiles: ride.distance || activeRide.distanceMiles || 0,
            durationMinutes:
              ride.estimatedDuration || activeRide.durationMinutes || 0,
            pickupDistance: 0,
            otp: ride.otp,
            paymentMethod: "card",
            walletDeduction: ride.walletDeduction || 0,
          ...
          console.warn(
            `⚠️ Failed to restore activeRideRequest for ride ${activeRide.id}:`,
            err,
          );
        }
      };
      ...
      if (user?.id) {
        try {
          ...
          if (res.ok) {
            ...
            if (driverId) {
              ...
              await fetch(`${baseUrl}/api/drivers/${driverId}`, {
                ...
                body: JSON.stringify({
                  ...
                  documentBankStatementStatus:
                    profile.documentBankStatementStatus,
                  documentDvlaCheckCodeUrl: profile.documentDvlaCheckCodeUrl,
                  documentDvlaCheckCodeStatus:
                    profile.documentDvlaCheckCodeStatus,
                  documentNationalInsuranceUrl:
                    profile.documentNationalInsuranceUrl,
                  documentNationalInsuranceStatus:
                    profile.documentNationalInsuranceStatus,
                  ...
                  documentProfilePhotoStatus:
                    profile.documentProfilePhotoStatus,
                }),
              });
            ...
            console.warn(
              "⚠️ Could not fetch driver record from Supabase to update vehicle info",
            );
          }
        } catch (apiErr) {
      ...
      if (rideState === "accepted" || rideState === "at_pickup") {
        ...
        if (isAtPickup && activeRideRequest.estimatedFare > 0) {
          ...
          setDriverDeductions((prev) => [
            localDeduction,
            ...prev.filter((d) => d.id !== localDeduction.id),
          ]);
          setTimeout(() => {
            refreshData().catch((err: any) =>
              console.warn("⚠️ Post-cancel refreshData failed:", err),
            );
          }, 2000);
        }
  ...
  const arrivedAtPickup = () => {
    ...
    (async () => {
      try {
        ...
        const normalizedRideType = String(rideType)
          .toLowerCase()
          .replace(/[\s-]+/g, "_");
        const matchingVehicleKey = Object.keys(vehiclePricing || {}).find(
          (key) =>
            String(key)
              .toLowerCase()
              .replace(/[\s-]+/g, "_") === normalizedRideType,
        );
        const waitPrice = parseFloat(
          (matchingVehicleKey
            ? vehiclePricing?.[matchingVehicleKey]?.waiting_price
            : undefined) || "0.50",
        );
        setWaitingChargePerMin(waitPrice > 0 ? waitPrice : 0.5);
        console.log(`⏱️ Waiting charge rate: £${waitPrice}/min`);
      } catch (e) {
        console.warn("Could not fetch waiting price, using default £0.50/min");
        setWaitingChargePerMin(0.5);
      }
    })();
  };
  ...
  const noShowRide = () => {
    ...
    if (fare > 0) {
      ...
      setTripHistory((prev) => {
        ...
        AsyncStorage.setItem(TRIP_HISTORY_KEY, JSON.stringify(updated)).catch(
          console.error,
        );
        return updated;
      });
  ...
  const agreeToWait = () => {
    ...
    console.log(
      `⏱️💰 Paid waiting started at ${now} for ride ${activeRideRequest.id}`,
    );
  ...
  const startRide = async (rideId: string, otp: string): Promise<boolean> => {
    if (
      activeRideRequest &&
      activeRideRequest.otp &&
      activeRideRequest.otp === otp
    ) {
      try {
        const socket = getSocket();
        socket.emit("ride:status", {
  ...
  const completeTrip = async (earlyCompletionReason?: string) => {
    if (activeRideRequest) {
      ...
      if (paidWaitingStartedAt) {
        const paidWaitingMs =
          Date.now() - new Date(paidWaitingStartedAt).getTime();
        const paidWaitingMin = Math.max(0, Math.floor(paidWaitingMs / 60000));
        waitingCharge =
          Math.round(paidWaitingMin * waitingChargePerMin * 100) / 100;
        console.log(
          `⏱️💰 Paid waiting: ${paidWaitingMin} mins × £${waitingChargePerMin} = £${waitingCharge}`,
        );
      }

      const totalFare =
        Math.round((activeRideRequest.estimatedFare + waitingCharge) * 100) /
        100;
  ...
  const dismissPaymentCollection = (
    collectedAmount?: number,
    extraAmount?: number,
  ) => {
    ...
    setTimeout(() => {
      ...
      refreshData().catch((err) =>
        console.warn("⚠️ Post-payment refreshData failed:", err),
      );
    ...
    setTimeout(() => {
      ...
      refreshData().catch((err) =>
        console.warn("⚠️ Post-payment refreshData follow-up failed:", err),
      );
    }, 5000);
  };
  ...
  const calculateEarnings = (): Earnings => {
    ...
    return {
      ...
      averageRating:
        ratedTrips > 0 ? Math.round((totalRating / ratedTrips) * 10) / 10 : 0.0,
    };
  };
