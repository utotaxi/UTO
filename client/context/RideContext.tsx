...
import { getApiUrl } from "@/lib/query-client";
import type { FareQuoteInput, FareQuote } from "@/lib/api";
...
interface RideContextType {
  ...
  updateRidePaymentMethod: (rideId: string, method: string) => Promise<void>;
  /**
   * Authoritative fare quote for pricing previews. The server proxies to the
   * admin pricing endpoint; at booking time the server re-quotes again.
   */
  getFareQuote: (input: FareQuoteInput) => Promise<FareQuote>;
  refreshRideHistory: () => Promise<void>;
  pendingRating: { rideId: string; driverName: string } | null;
  submitRiderRating: (rideId: string, rating: number, comment?: string) => void;
  dismissRiderRating: () => void;
...
export function RideProvider({ children }: { children: ReactNode }) {
  ...
  // Authoritative fare quote — proxies to the admin pricing endpoint via the
  // app's own /api/quote route. Used for previews; the server re-quotes again
  // at booking time so a client-supplied price is never stored.
  const getFareQuote = useCallback(
    async (input: FareQuoteInput): Promise<FareQuote> => {
      const { api } = await import("@/lib/api");
      return api.quotes.get(input);
    },
    [],
  );
  ...
  const requestRide = async (
    ...
    // Preview fare comes from the admin quote endpoint (server re-quotes again
    // authoritatively in handleRideRequest). Never compute the fare locally.
    ...
    const fullFare = Math.max(0, quote.price);
