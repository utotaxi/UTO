...
//   rideCancelledByDriver: boolean;
//   clearDriverCancelNotification: () => void;
//   isLoading: boolean;
// }
...
//   const [rideCancelledByDriver, setRideCancelledByDriver] = useState(false);
//   // Ref to track if the RIDER initiated the cancel (so we don't show "driver cancelled" popup for their own action)
//   const riderInitiatedCancelRef = React.useRef(false);
...
//   // ─── Re-register rider when the socket reconnects ─────────────────────────
//   // If the socket drops and reconnects (e.g. server restart, network blip),
//   // re-announce the rider so the server can route ride updates back here.
//   useEffect(() => {
//     if (!user?.id) return;
...
//       socket = getSocket();
//     } catch (err) {
//       console.warn('⚠️ RideContext: getSocket failed in reconnect effect:', err);
//       return;
//     }
...
//       try {
//         connectAsRider(user.id);
//         console.log('🔁 RideContext: socket reconnected and re-registered rider', user.id);
//       } catch (err) {
//         console.warn('⚠️ RideContext: reconnect handler failed:', err);
//       }
//     };
...
//               const currentBalance = userRef.current?.walletBalance || 0;
//               updateProfile({ walletBalance: currentBalance - noShowFare }); // allow negative per policy
//               console.log(`❌ [RideContext] No-show penalty: £${noShowFare} debited from wallet (may go negative)`);
//             }
...
//           // ── Detect driver-initiated cancellation ──────────────────────────
//           // If it's a plain "cancelled" update AND the rider didn't trigger it, the driver cancelled.
//           if (update.status === "cancelled" && !riderInitiatedCancelRef.current) {
//             setRideCancelledByDriver(true);
//           }
//           riderInitiatedCancelRef.current = false; // always reset
...
//     // The prompt requested 'waiting time etc' to be considered. We'll add base_minute_price to duration.
...
//     // Mark rider-initiated before emitting so the socket listener doesn't show "driver cancelled" popup
//     riderInitiatedCancelRef.current = true;
...
//         try {
//           if (penaltyAmount > 0) {
//             // Allow balance to go negative — policy requires 100% fare charged even if insufficient balance
//             const newBalance = currentBalance - penaltyAmount;
//             await updateProfile({ walletBalance: newBalance });
//             console.log(`✅ [RideContext] Deducted £${penaltyAmount} penalty for cancelled ride ${rideId}. New balance: £${newBalance.toFixed(2)}`);
//           }
...
//       try {
//         const socket = getSocket();
//         socket.emit("ride:status", { rideId, status: "cancelled", cancelledBy: "rider" });
//       } catch (err) {
...
//   const clearDriverCancelNotification = () => {
//     setRideCancelledByDriver(false);
//   };
