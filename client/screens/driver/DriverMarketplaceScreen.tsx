...
function BookingCard({
  ...
  const driverOwnsThis =
    item.driver_id === driverId || item.driver_id === driverId;
    ...
    <Pressable style={s.card} onPress={() => onPress(item)}>
      <View style={s.cardHeader}>
        <View
          style={[
            s.statusBadge,
            item.status === "cancelled" && s.statusBadgeCancelled,
          ]}
        >
          <Text
            style={[
              s.statusText,
              item.status === "cancelled" && s.statusTextCancelled,
            ]}
          >
            {item.isUrgentScheduled
      ...
      <View style={s.detailsContainer}>
        ...
        <View style={s.detailRow}>
          ...
          <Text style={s.detailValue}>
            {String(item.payment_method || "card").toLowerCase() === "cash"
              ? "Cash"
              : "Card / App"}
          </Text>
        </View>
      ...
      <View style={s.cancellationPolicyBox}>
        <Text style={s.deadlineText}>
          Accepted bookings stay assigned to you until pickup.
        </Text>
        <Text style={s.lateText}>
          If you cancel, the booking is released for another driver.
        </Text>
      </View>

      {/* Late cancel warning for driver */}
      {item.status === "driver_accepted" &&
        driverOwnsThis &&
        isLateCancelWindow &&
        item.estimated_fare && (
          <View style={s.lateCancelWarning}>
            <Text style={s.lateCancelText}>
              Cancelling close to pickup releases this booking for ASAP
              dispatch.
            </Text>
          </View>
        )}
      ...
      [
        { text: "Back", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              const res = await fetch(
                `${getApiUrl()}/api/later-bookings/${id}/accept`,
                {
                  method: "PUT",
        ...
        { text: "Keep It", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              const res = await fetch(
                `${getApiUrl()}/api/later-bookings/${item.id}/cancel`,
                {
                  method: "PUT",
                  headers: { "Content-Type": "application/json" },
    ...
    <View style={[s.root, { paddingTop: insets.top }]}>
      ...
      <View style={s.policyBanner}>
        ...
        <Text style={s.policyText}>
          If you cancel an accepted booking, it will be released back to
          marketplace or ASAP dispatch for another driver.
        </Text>
      </View>
      ...
      {loading ? (
        ...
        <View style={s.centered}>
          ...
          <Text style={s.emptyText}>
            When riders book a later ride, they'll appear here.
          </Text>
        </View>
      ) : (
        <FlatList
