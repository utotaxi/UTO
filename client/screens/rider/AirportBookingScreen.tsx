...
export default function AirportBookingScreen({ navigation }: any) {
  ...
  const isSaloonEligible =
    (passengers <= 3 && luggage <= 3) || (passengers <= 4 && luggage === 0);
  const isCarrierEligible =
    (passengers <= 5 && luggage <= 5) || (passengers <= 6 && luggage === 0);
  const isMinibusEligible = passengers <= 8 && luggage <= 8;

  // Return vehicle eligibility (based on return passengers & luggage)
  const isReturnSaloonEligible =
    (returnPassengers <= 3 && returnLuggage <= 3) ||
    (returnPassengers <= 4 && returnLuggage === 0);
  const isReturnCarrierEligible =
    (returnPassengers <= 5 && returnLuggage <= 5) ||
    (returnPassengers <= 6 && returnLuggage === 0);
  ...
  const maxDate = React.useRef(
    new Date(openedAt.getTime() + 365 * 24 * 60 * 60 * 1000),
  ).current;
  ...
  const [returnSelectedVehicle, setReturnSelectedVehicle] =
    useState<VehicleType>("saloon");
  ...
  useEffect(() => {
    if (isSaloonEligible) {
      if (
        ...
        selectedVehicle !== "people_carrier" &&
        selectedVehicle !== "minibus"
      )
        setSelectedVehicle("saloon");
    } else if (isCarrierEligible) {
      if (selectedVehicle === "saloon") setSelectedVehicle("people_carrier");
    } else if (isMinibusEligible) {
  ...
  useEffect(() => {
    if (isReturnSaloonEligible) {
      /* keep current if eligible */
    } else if (isReturnCarrierEligible) {
      if (returnSelectedVehicle === "saloon")
        setReturnSelectedVehicle("people_carrier");
    } else if (isReturnMinibusEligible) {
      setReturnSelectedVehicle("minibus");
    }
  }, [returnPassengers, returnLuggage]);

  const [returnFlightNumber, setReturnFlightNumber] = useState("");
  const [returnEstimatedFare, setReturnEstimatedFare] = useState<number | null>(
    null,
  );
  ...
  const [returnHourVal, setReturnHourVal] = useState(() =>
    returnSelectedDate.getHours(),
  );
  ...
  const canReturnNavPrev =
    returnCalendarMonth.year > todayMonth.year ||
    (returnCalendarMonth.year === todayMonth.year &&
      returnCalendarMonth.month > todayMonth.month);
  const canReturnNavNext =
    returnCalendarMonth.year < maxMonth.year ||
    (returnCalendarMonth.year === maxMonth.year &&
      returnCalendarMonth.month < maxMonth.month);
  ...
  // ── Quote fare when both locations are set or vehicle changes ──
  useEffect(() => {
    if (!pickupLocation || !dropoffLocation) {
      setEstimatedFare(null);
      setReturnEstimatedFare(null);
    ...
    const calculateFare = async () => {
      ...
      try {
        ...
        // Authoritative previews from the admin quote endpoint. The server
        // re-quotes each leg again at booking time. No local fare fallback.
        const [outboundQuote, returnQuote] = await Promise.all([
          getFareQuote({
            ...
            vehicleType: fareVehicle(selectedVehicle),
            minutes: dur,
          }),
          getFareQuote({
            pickupLat: pickupLocation.latitude,
            ...
            dropoffLat: dropoffLocation.latitude,
            dropoffLng: dropoffLocation.longitude,
            vehicleType: fareVehicle(returnSelectedVehicle),
            minutes: dur,
          }),
        ]);
        setEstimatedFare(Math.max(0, outboundQuote.price));
        setReturnEstimatedFare(Math.max(0, returnQuote.price));
      } catch (err: any) {
        console.warn("Failed to calculate fare:", err);
        setEstimatedFare(null);
        setReturnEstimatedFare(null);
        setPricingError(
          err?.message || "Pricing unavailable — contact dispatch",
        );
      } finally {
  ...
  const renderReturnCalendar = () => {
    const firstDay = new Date(
      returnCalendarMonth.year,
      returnCalendarMonth.month,
      1,
    ).getDay();
    const daysInMonth = new Date(
      returnCalendarMonth.year,
      returnCalendarMonth.month + 1,
      0,
    ).getDate();
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      const isSelected =
        cellDate.toDateString() === returnSelectedDate.toDateString();
        ...
        >
          <Text
            style={[
              ...
              disabled && s.calCellTextDisabled,
            ]}
          >
            {d}
          </Text>
  ...
  const handleContinue = async () => {
    ...
    if (!flightNumber || flightNumber.trim() === "") {
      Alert.alert(
        ...
        "Please enter your flight number before booking your airport transfer.",
      );
      return;
    }

    if (pricingError || estimatedFare == null) {
      Alert.alert(
        "Pricing unavailable",
        pricingError || "Pricing unavailable — contact dispatch",
      );
      return;
    }
    ...
    if (scheduledTime <= rightNow) {
      Alert.alert(
        "Time has passed",
        `Scheduled time ${fmtTime(scheduledTime)} is in the past. Please select a future time.`,
      );
      return;
    }
    ...
    if (isReturnJourney) {
      if (!returnPickup || returnPickup.trim() === "") {
        Alert.alert(
          "Missing return pickup",
          "Please enter a pickup location for your return journey.",
        );
        return;
      }
      if (!returnDropoff || returnDropoff.trim() === "") {
        Alert.alert(
          "Missing return drop-off",
          "Please enter a drop-off location for your return journey.",
        );
        return;
      }
      if (!returnFlightNumber || returnFlightNumber.trim() === "") {
        Alert.alert(
          "Missing return flight number",
          "Please enter your return flight number before booking your airport transfer.",
        );
        return;
      }
      ...
      if (returnScheduledTime <= scheduledTime) {
        Alert.alert(
          "Invalid return time",
          "Return journey time must be after the outbound journey.",
        );
        return;
      }
      const returnTimeDiffMs =
        returnScheduledTime.getTime() - rightNow.getTime();
    ...
    try {
      const collectScheduledPayment = async (amount: number, label: string) => {
        ...
        if (initError) {
          throw new Error(
            initError.message || `Could not start ${label} card payment.`,
          );
        }
        ...
        if (presentError) {
          throw new Error(
            presentError.message ||
              `Please complete ${label} card payment to schedule this ride.`,
          );
        }
      ...
      if (
        ...
        returnPaymentIntentId = await collectScheduledPayment(
          returnEstimatedFare,
          "return",
        );
      }
      ...
      if (isReturnJourney && returnScheduledTime) {
        ...
        try {
          returnResBody = await returnRes.json();
        } catch (_) {}

        if (!returnRes.ok) {
          throw new Error(
            returnResBody.error ||
              `Server error on return booking ${returnRes.status}`,
          );
        }
      ...
      Alert.alert(
        "Error",
        err.message || "Failed to save booking. Please try again.",
      );
    } finally {
      setIsSaving(false);
    }
    ...
    >
      ...
      >
        ...
        <View style={s.counterSection}>
            ...
            <View style={s.counterControls}>
              ...
              >
                <MaterialIcons
                  name="remove"
                  size={20}
                  color={passengers <= 1 ? "#D1D5DB" : "#374151"}
                />
              </Pressable>
              ...
              >
                <MaterialIcons
                  name="add"
                  size={20}
                  color={passengers >= 8 ? "#D1D5DB" : "#374151"}
                />
              </Pressable>
            </View>
          </View>
          ...
          <View style={s.counterRow}>
            ...
            <View style={s.counterControls}>
              <Pressable
                style={[s.counterBtn, luggage <= 0 && s.counterBtnDisabled]}
                onPress={() => {
                  if (luggage > 0) {
                    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                    setLuggage((l) => l - 1);
              ...
              >
                <MaterialIcons
                  name="remove"
                  size={20}
                  color={luggage <= 0 ? "#D1D5DB" : "#374151"}
                />
              </Pressable>
              <Text style={s.counterValue}>{luggage}</Text>
              <Pressable
                style={[s.counterBtn, luggage >= 8 && s.counterBtnDisabled]}
                onPress={() => {
                  if (luggage < 8) {
                    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                    setLuggage((l) => l + 1);
          ...
          <View style={{ gap: 10 }}>
            {isSaloonEligible && (
              <Pressable
                style={[
                  s.vehicleOption,
                  selectedVehicle === "saloon" && s.vehicleOptionActive,
              ...
              >
                ...
                <View style={{ marginLeft: 12, flex: 1 }}>
                  <Text
                    style={[
                      s.vehicleOptionName,
                      selectedVehicle === "saloon" && s.vehicleOptionNameActive,
                      { marginTop: 0 },
                    ]}
                  >
                    Saloon Car
                  ...
                  <Text
                    style={[
                      s.vehicleOptionDesc,
                      selectedVehicle === "saloon" && s.vehicleOptionDescActive,
                    ]}
                  >
                    Up to 3 passengers + 3 baggage, or 4 passengers + hand
                    luggage
                  </Text>
                </View>
              </Pressable>
            ...
            {isCarrierEligible && (
              <Pressable
                style={[
                  s.vehicleOption,
                  selectedVehicle === "people_carrier" && s.vehicleOptionActive,
                  { flexDirection: "row", paddingHorizontal: 16 },
                ]}
                onPress={() => {
                  Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                  setSelectedVehicle("people_carrier");
                }}
              >
                <MaterialIcons
                  name="directions-car"
                  size={24}
                  color={
                    selectedVehicle === "people_carrier" ? "#000000" : "#6B7280"
                  }
                />
                <View style={{ marginLeft: 12, flex: 1 }}>
                  <Text
                  ...
                  <Text
                    style={[
                      s.vehicleOptionDesc,
                      selectedVehicle === "people_carrier" &&
                        s.vehicleOptionDescActive,
                    ]}
                  >
                    Up to 5 passengers + 5 baggage, or 6 passengers + hand
                    luggage
                  </Text>
                </View>
              </Pressable>
            ...
            {isMinibusEligible && (
              <Pressable
                style={[
                  s.vehicleOption,
                  selectedVehicle === "minibus" && s.vehicleOptionActive,
                  { flexDirection: "row", paddingHorizontal: 16 },
                ]}
                onPress={() => {
                  Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
            ...
            {!isSaloonEligible && !isCarrierEligible && !isMinibusEligible && (
              ...
              >
                No vehicles available for this combination. Please reduce
        ...
        >
          <View style={s.roundTripLeft}>
            ...
            <View>
              <Text
                style={[
                  s.roundTripTitle,
                  isReturnJourney && s.roundTripTitleActive,
                ]}
              >
                Return Journey
              </Text>
              <Text
                style={[
                  s.roundTripSub,
                  isReturnJourney && s.roundTripSubActive,
                ]}
              >
                Book return journey
              </Text>
          ...
          <View
            style={[
              s.roundTripToggle,
              isReturnJourney && s.roundTripToggleActive,
            ]}
          >
            <View
              style={[s.roundTripDot, isReturnJourney && s.roundTripDotActive]}
            />
          </View>
        </Pressable>
        ...
        <View style={s.timeCard}>
          ...
          <View style={s.spinnerRow}>
            ...
            <View style={s.spinnerCol}>
              <Pressable
                onPress={() =>
                  setMinuteVal((m) => (Math.round(m / 5) * 5 + 5) % 60)
                }
                hitSlop={{ top: 12, bottom: 12, left: 24, right: 24 }}
              >
                <MaterialIcons
              ...
              <Pressable
                onPress={() =>
                  setMinuteVal((m) => (Math.round(m / 5) * 5 - 5 + 60) % 60)
                }
                hitSlop={{ top: 12, bottom: 12, left: 24, right: 24 }}
              >
                <MaterialIcons
        ...
        {isReturnJourney && (
          <View style={[s.timeCard, { marginTop: -8 }]}>
            ...
            <View style={{ paddingHorizontal: 16, paddingBottom: 16 }}>
              <View
                style={[s.locationCard, { marginHorizontal: 0, marginTop: 0 }]}
              >
                <View style={s.locationRow}>
                  <View style={s.routeDot} />
                  <View style={{ flex: 1 }}>
              ...
              <View style={s.counterRow}>
                ...
                <View style={s.counterControls}>
                  <Pressable
                    style={[
                      s.counterBtn,
                      returnPassengers <= 1 && s.counterBtnDisabled,
                    ]}
                    onPress={() => {
                      if (returnPassengers > 1) {
                        Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                        setReturnPassengers((p) => p - 1);
                      }
                    }}
                  >
                    <MaterialIcons
                      name="remove"
                      size={20}
                      color={returnPassengers <= 1 ? "#D1D5DB" : "#374151"}
                    />
                  </Pressable>
                  ...
                  <Pressable
                    style={[
                      s.counterBtn,
                      returnPassengers >= 8 && s.counterBtnDisabled,
                    ]}
                    onPress={() => {
                      if (returnPassengers < 8) {
                        Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                        setReturnPassengers((p) => p + 1);
                      }
                    }}
                  >
                    <MaterialIcons
                      name="add"
                      size={20}
                      color={returnPassengers >= 8 ? "#D1D5DB" : "#374151"}
                    />
                  </Pressable>
                </View>
              </View>
              ...
              <View style={s.counterRow}>
                ...
                <View style={s.counterControls}>
                  <Pressable
                    style={[
                      s.counterBtn,
                      returnLuggage <= 0 && s.counterBtnDisabled,
                    ]}
                    onPress={() => {
                      if (returnLuggage > 0) {
                        Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                        setReturnLuggage((l) => l - 1);
                      }
                    }}
                  >
                    <MaterialIcons
                      name="remove"
                      size={20}
                      color={returnLuggage <= 0 ? "#D1D5DB" : "#374151"}
                    />
                  </Pressable>
                  ...
                  <Pressable
                    style={[
                      s.counterBtn,
                      returnLuggage >= 8 && s.counterBtnDisabled,
                    ]}
                    onPress={() => {
                      if (returnLuggage < 8) {
                        Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                  ...
                  >
                    <MaterialIcons
                      name="add"
                      size={20}
                      color={returnLuggage >= 8 ? "#D1D5DB" : "#374151"}
                    />
                  </Pressable>
                </View>
              </View>
            ...
            >
              ...
              <View style={{ gap: 8 }}>
                {isReturnSaloonEligible && (
                  <Pressable
                    style={[
                      s.vehicleOption,
                      returnSelectedVehicle === "saloon" &&
                        s.vehicleOptionActive,
                    ]}
                    onPress={() => {
                      Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                      setReturnSelectedVehicle("saloon");
                    }}
                  >
                    <Text
                      style={[
                ...
                {isReturnCarrierEligible && (
                  <Pressable
                    style={[
                      s.vehicleOption,
                      returnSelectedVehicle === "people_carrier" &&
                        s.vehicleOptionActive,
                    ]}
                    onPress={() => {
                      Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                ...
                {isReturnMinibusEligible && (
                  <Pressable
                    style={[
                      s.vehicleOption,
                      returnSelectedVehicle === "minibus" &&
                        s.vehicleOptionActive,
                    ]}
                    onPress={() => {
                      Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
            ...
            <TouchableOpacity
              style={s.dateHeader}
              onPress={() => setShowReturnCalendar(!showReturnCalendar)}
              activeOpacity={0.85}
            >
              <Text style={s.dateHeaderYear}>
                {returnSelectedDate.getFullYear()}
              </Text>
              <Text style={s.dateHeaderDate}>
                {fmtDate(returnSelectedDate)}
              </Text>
            </TouchableOpacity>
            {showReturnCalendar && (
              <View style={s.calBox}>
                ...
                <View style={s.calFooter}>
                  <Pressable onPress={() => setShowReturnCalendar(false)}>
                    <Text style={s.calCancel}>CANCEL</Text>
                  </Pressable>
                  <Pressable onPress={() => setShowReturnCalendar(false)}>
                    <Text style={s.calOk}>OK</Text>
            ...
            <View style={s.spinnerRow}>
              <View style={s.spinnerCol}>
                <Pressable
                  onPress={() => setReturnHourVal((h) => (h + 1) % 24)}
                  hitSlop={{ top: 12, bottom: 12, left: 24, right: 24 }}
                >
                  <MaterialIcons
                    name="keyboard-arrow-up"
                ...
                <Text style={s.spinnerVal}>
                  {String(returnHourVal).padStart(2, "0")}
                </Text>
                <Pressable
                  onPress={() => setReturnHourVal((h) => (h - 1 + 24) % 24)}
                  hitSlop={{ top: 12, bottom: 12, left: 24, right: 24 }}
                >
                  <MaterialIcons
                    name="keyboard-arrow-down"
              ...
              <View style={s.spinnerCol}>
                <Pressable
                  onPress={() =>
                    setReturnMinuteVal((m) => (Math.round(m / 5) * 5 + 5) % 60)
                  }
                  hitSlop={{ top: 12, bottom: 12, left: 24, right: 24 }}
                >
                  <MaterialIcons
                ...
                <Text style={s.spinnerVal}>
                  {String(returnMinuteVal).padStart(2, "0")}
                </Text>
                <Pressable
                  onPress={() =>
                    setReturnMinuteVal(
                      (m) => (Math.round(m / 5) * 5 - 5 + 60) % 60,
                    )
                  }
                  hitSlop={{ top: 12, bottom: 12, left: 24, right: 24 }}
                >
        ...
        {(estimatedFare !== null ||
          ...
          <View style={s.fareCard}>
            ...
            {isCalculatingFare ? (
              ...
              <Text style={[s.fareCardPrice, { fontSize: 15 }]}>
                {pricingError}
              </Text>
            ) : (
              <>
                <Text style={s.fareCardPrice}>
                  ...
                  {Math.max(
                    ...
                    (!isReturnJourney
                      ? estimatedFare!
                      : estimatedFare! + (returnEstimatedFare || 0)) -
                      couponDiscount,
                  ).toFixed(2)}
                </Text>
                {couponDiscount > 0 && (
                  <Text
                    style={{ fontSize: 13, color: "#059669", marginBottom: 4 }}
                  >
      ...
      <View style={[s.footer, { paddingBottom: insets.bottom + 16 }]}>
        <TouchableOpacity
          style={[
            s.continueBtn,
            (isSaving || isCalculatingFare || pricingError != null) && {
              opacity: 0.7,
            },
          ]}
          onPress={handleContinue}
          activeOpacity={0.85}
          disabled={isSaving || isCalculatingFare || pricingError != null}
        >
          {isSaving ? (
            ...
            <Text style={s.continueBtnText}>
              {(() => {
                ...
                const total = estimatedFare
                  ...
                  : null;
                return total
                  ? `Book ${vn}${isReturnJourney ? " + Return" : ""} · £${total.toFixed(2)}`
                  : `Book ${vn}${isReturnJourney ? " + Return" : ""} Transfer`;
              })()}
            </Text>
          )}
        </TouchableOpacity>
