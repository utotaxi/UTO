...
export default function RideRequestScreen({ navigation, route }: any) {
  const insets = useSafeAreaInsets();
  const { isDark } = useTheme();
  const { activeRide, requestRide, getFareQuote } = useRide();
  const { user } = useAuth();
  const { initPaymentSheet, presentPaymentSheet } = useStripe();
  ...
  const [distanceKm, setDistanceKm] = useState<number | null>(null);
  const [durationMin, setDurationMin] = useState<number | null>(null);
  // Per-vehicle fare quotes from the admin pricing endpoint (previews).
  // undefined = not yet loaded / unavailable; the server re-quotes at booking.
  const [priceQuotes, setPriceQuotes] = useState<
    Partial<Record<RideType, number>>
  >({});
  ...
  const handleValidateCoupon = async () => {
    ...
    try {
      const fareAmount = calculatePrice(selectedVehicle);
      if (fareAmount == null) {
        setCouponError("Price is still loading — try again in a moment");
        return;
      }
  ...
  // ── Fetch authoritative fare quotes for each vehicle type (previews) ──
  // The server re-quotes again at booking time; these are display-only.
  useEffect(() => {
    if (
      !pickupLocation ||
      !dropoffLocation ||
    ...
    const minutes = durationMin || 15;

    let cancelled = false;
    (async () => {
      setPricingError(null);
      // Clear stale prices so a previous value doesn't show for new inputs.
      setPriceQuotes({});
      try {
        const results = await Promise.all(
          VEHICLE_OPTIONS.map((v) =>
            getFareQuote({
              ...
              vehicleType: v.type,
              minutes,
            }).then((q) => [v.type, Math.max(0, q.price)] as const),
          ),
        );
        if (cancelled) return;
        const next: Partial<Record<RideType, number>> = {};
        for (const [type, price] of results) {
          next[type] = price;
        }
        setPriceQuotes(next);
      } catch (err: any) {
  ...
  }, [pickupLocation, dropoffLocation, durationMin, getFareQuote]);

  /** Returns the cached quote for a vehicle type, or undefined while loading. */
  const calculatePrice = (type: RideType): number | undefined => {
    return priceQuotes[type];
  };
  ...
  const handleRequestRide = async () => {
    ...
    if (pricingError || calculatePrice(selectedVehicle) == null) {
      Alert.alert(
        "Pricing unavailable",
    ...
    try {
      ...
      console.error("Failed to request ride:", error);
      const msg = error?.message || "Pricing unavailable — contact dispatch";
      Alert.alert(
        "Couldn't request ride",
        msg.includes("Pricing unavailable")
          ? msg
                        ...
                        <View style={{ alignItems: "flex-end", marginLeft: 8 }}>
                          ...
                          >
                            {price == null
                              ? "…"
                              : formatPrice(
                                  Math.max(0, price - couponDiscount),
                                )}
                          </ThemedText>
                        </View>
                      </Pressable>
            ...
            >
              {isRequesting ? (
                ...
                <>
                  ...
                  <ThemedText style={styles.requestButtonPrice}>
                    {(() => {
                      const p = calculatePrice(selectedVehicle);
                      return p == null
                        ? "…"
                        : formatPrice(Math.max(0, p - couponDiscount));
                    })()}
                  </ThemedText>
                </>
              )}
