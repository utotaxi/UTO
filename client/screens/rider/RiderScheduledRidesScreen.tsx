...
function RideCard({
  ...
  // Rider always sees the discounted fare stored on the booking. The stored
  // estimated_fare is now always set server-side from the admin quote; we do
  // not recalculate from distance client-side (no access to pricing rules).
  const displayFare =
    ride.estimated_fare != null
      ? Math.max(0, Number(ride.estimated_fare))
      : undefined;
    ...
    <View style={[cs.root, { paddingTop: insets.top }]}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <RideCard ride={item} onCancel={handleCancel} />
          )}
