...
export default function LaterRideScreen({ navigation }: any) {
  ...
  useEffect(() => {
    ...
    const calculateFare = async () => {
      ...
      try {
        ...
        // Authoritative preview comes from the admin quote endpoint (the
        // server re-quotes again at booking time). No local fare fallback.
        ...
        setEstimatedFare(Math.max(0, quote.price));
      } catch (err: any) {
    ...
    calculateFare();
  }, [pickupLocation, dropoffLocation, selectedVehicle, vias, getFareQuote]);
  ...
  const handleContinue = async () => {
    ...
    if (finalPickup <= rightNow) {
      Alert.alert(
        "Time has passed",
        `Pickup time ${fmtTime(finalPickup)} is in the past. Please select a future time.`,
      );
      return;
    }

    if (finalPickup.getTime() - rightNow.getTime() < 4 * 60 * 60 * 1000) {
    ...
    try {
      ...
      if (!res.ok) {
        const message = resBody.error || `Server error ${res.status}`;
      ...
      >
        ...
        <View style={s.fareCard}>
          <View style={s.fareCardHeader}>
            ...
            <Text style={s.fareCardTitle}>Discount Coupon</Text>
          </View>
          {isCouponApplied ? (
            <View
              style={{
            ...
            {isCalculatingFare ? (
              ...
              <>
                <Text style={s.fareCardPrice}>
                  £{Math.max(0, estimatedFare! - couponDiscount).toFixed(2)}
                </Text>
                {couponDiscount > 0 && (
                  <Text
                ...
                {distanceKm !== null && durationMin !== null && (
                  <Text style={s.fareCardSub}>
                    {(distanceKm * 0.621371).toFixed(1)} miles · ~{durationMin}{" "}
                    min ·{" "}
                    {selectedVehicle === "saloon"
                      ? "Saloon"
                      : selectedVehicle === "people_carrier"
