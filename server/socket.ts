...
import { getFareQuote, PricingUnavailableError } from "./services/pricing";
...
async function assertRideStillOfferable(rideId: string): Promise<boolean> {
  try {
    ...
    // Only select columns required for the offerability decision.
...
export function setupSocketIO(httpServer: HTTPServer) {
  ...
  const activeRides = new Map<
    string,
    {
      ...
      acceptedAt?: string;
      /** ISO timestamp when the driver marked arrived — anchors the 1-min
       * post-arrival free-cancel window for the cancel-fee decision. */
      arrivedAt?: string;
  ...
  const handleRideRequest = async (
    ...
    if (rideData.id) {
      ...
      if (rideData.riderId) {
        try {
          ...
          // endpoint. Never persist a client-supplied farePrice. If quoting
          // fails, reject the ride request so no booking is created.
          let serverFarePrice = 0;
          ...
          try {
            const quote = await getFareQuote({
              ...
              vehicleType: normalizedRideVehicleType,
              minutes: Number(rideData.durationMinutes) || 0,
            });
            serverFarePrice = Math.max(0, quote.price);
            billedMiles = quote.billedMiles || null;
          } catch (quoteErr) {
            console.error(
              "❌ getFareQuote failed for ride request",
              rideData.id,
              quoteErr,
            );
            if (sourceSocketId) {
              io.to(sourceSocketId).emit("ride:update", {
                ...
                status: "error",
                message:
                  quoteErr instanceof PricingUnavailableError
                    ? quoteErr.message
          ...
          const insertPayload: Record<string, any> = {
            ...
            estimated_price: serverFarePrice,
    ...
    if (rideData.id) {
      ...
      activeRides.set(rideData.id, {
        riderSocketId: riderSocketId || existing?.riderSocketId || "",
  ...
  io.on("connection", (socket: Socket) => {
      ...
      async (data: {
        ...
        try {
          ...
          if (rideFetchErr || !ride) {
            console.error(
              `❌ ride:driver_cancel_at_pickup — could not load ride ${data.rideId}:`,
              rideFetchErr?.message || "ride not found",
            );
            return;
          }
          ...
          try {
            ...
            const neverAssigned =
              !hadAssignment && !ageRow?.accepted_at && !ageRow?.driver_id;
            if (
              neverAssigned &&
              Number.isFinite(createdMs) &&
              createdMs > 0 &&
          ...
          // ride:status) — a failed reset leaves status "accepted"/"arrived"
          // and assertRideStillOfferable silently blocks every rematch offer.
          const rematchRequestedAt = new Date().toISOString();
          let payloadToReset: Record<string, any> = {
            status: "pending",
            driver_id: null,
            accepted_at: null,
            arrived_at: null,
          ...
          for (let attempt = 0; attempt < 8; attempt += 1) {
            const result = await supabase
              .from("rides")
              .update(payloadToReset)
              .eq("id", data.rideId)
              .select("id, status, driver_id")
              .maybeSingle();
            resetRide = result.data;
            resetErr = result.error;
            if (!resetErr) break;

            const missingColumn = String(resetErr.message || "").match(
              /'([^']+)'/,
            )?.[1];
            if (
              missingColumn &&
              ...
              /column|42703|PGRST204/i.test(String(resetErr.message || ""))
            ) {
              delete payloadToReset[missingColumn];
              console.warn(
                `⚠️ rides.${missingColumn} missing — retrying rematch reset without it`,
              );
              continue;
            }
            break;
          ...
          // Last-resort core reset — only fields required for rematch dispatch.
          if (
            resetErr ||
            !resetRide ||
            String(resetRide.status || "").toLowerCase() !== "pending" ||
            resetRide.driver_id
          ) {
            const coreReset = await supabase
              .from("rides")
              .update({
                status: "pending",
                driver_id: null,
              ...
              .select("id, status, driver_id")
              .maybeSingle();
            if (!coreReset.error && coreReset.data) {
              resetRide = coreReset.data;
              resetErr = null;
            } else if (coreReset.error) {
              resetErr = coreReset.error;
            }
          }
          ...
          if (
            ...
            console.error(
              `❌ Rematch aborted — failed to reset ride ${data.rideId} to pending/unassigned:`,
              resetErr?.message ||
    ...
    socket.on(
      ...
      async (data: { rideId: string; driverId: string }) => {
        ...
        // Store the driver socket and identity for this ride in the active rides map
        const rideInfo = activeRides.get(data.rideId);
        if (rideInfo) {
          rideInfo.driverSocketId = socket.id;
          rideInfo.acceptedAt = acceptedAt;
          (rideInfo as any).acceptedDriverId = actualDriverId;
          (rideInfo as any).acceptedDriverRawId = data.driverId;
        }
    ...
    socket.on("ride:status", async (update: RideUpdate) => {
      ...
      try {
          ...
          if (update.status === "accepted") {
            ...
            try {
              ...
              if (
                ...
                updateData.arrived_at = new Date(arrivedAt).toISOString();
              }
        ...
        if (update.status === "arrived" || update.status === "at_pickup") {
          ...
          // Also stash it in memory so the cancel-fee branch can resolve the
          // arrival time even if the DB arrived_at write hasn't landed yet.
          if (rideInfo) {
