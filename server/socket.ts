...
// // ─── Resolve Driver Record ID ─────────────────────────────────────────────────
// // Clients sometimes send their auth user_id instead of the drivers table id.
// // This helper normalises any raw ID to the canonical drivers.id, with a fallback
// // to the raw value if neither lookup succeeds (avoids breaking callers).
// async function resolveDriverRecordId(rawDriverId: string): Promise<string> {
...
//       .from("drivers")
//       .select("id")
//       .eq("user_id", rawDriverId)
//       .single();
//     if (driverByUserId?.id) {
//       console.log(`🔄 Resolved driver auth user_id ${rawDriverId} → driver id ${driverByUserId.id}`);
//       return driverByUserId.id;
//     }
//   } catch (err) {
//     console.warn(`⚠️ Could not resolve driver ID for ${rawDriverId}:`, err);
//   }
//   return rawDriverId;
// }
...
//             const rawDriverId = data.driverId || ride.driver_id;
//             const actualDriverId = await resolveDriverRecordId(rawDriverId);
...
//       try {
//         // ─── Resolve to canonical drivers table id ───────────────────────────
//         const actualDriverId = await resolveDriverRecordId(data.driverId);
...
//         // Priority 5: Normalise to drivers table id via shared helper
//         // (handles the case where client sent auth user_id instead of drivers.id)
//         if (resolvedDriverId) {
//           resolvedDriverId = await resolveDriverRecordId(resolvedDriverId);
//         }
...
//       let rawPayingDriverId: string | null = null;
//       for (const [dId, sId] of connectedDrivers.entries()) {
//         if (sId === socket.id) { rawPayingDriverId = dId; break; }
//       }

//       // Fallback: get driver_id from the ride record itself
//       if (!rawPayingDriverId) {
//         try {
//           const { data: existingRide } = await supabase
//             .from("rides")
//             .select("driver_id")
...
//             .single();
//           if (existingRide?.driver_id) {
//             rawPayingDriverId = existingRide.driver_id;
//             console.log(`🔍 payment_collected — Resolved driver_id from ride record: ${rawPayingDriverId}`);
//           }
//         } catch (_) {
//           // Non-critical
//         }
...
//       // ─── Normalise to canonical drivers table id via shared helper ──────────
//       const payingDriverId = rawPayingDriverId
//         ? await resolveDriverRecordId(rawPayingDriverId)
//         : null;
...
export function setupSocketIO(httpServer: HTTPServer) {
  ...
  const resolveDriverTableId = async (driverId?: string | null): Promise<string | null> => {
    ...
    return driverByUserId?.id || driverId;
  };
  ...
  io.on("connection", (socket: Socket) => {
    ...
    socket.on("driver:connect", async (driverId: string) => {
      ...
      const connectedDriverId = actualDriverId || driverId;
      ...
      try {
        ...
        if (rideErr || !ride) {
          ...
          const storedLocation =
            (driverId ? latestDriverLocations.get(driverId) : null) ||
            (rideInfo.driverSocketId ? latestDriverLocations.get(rideInfo.driverSocketId) : null);
          const payload = storedLocation
            ? { ...storedLocation, rideId: data.rideId }
            : buildFallbackDriverLocation(data.rideId, driverId);

          if (payload) {
            io.to(socket.id).emit("driver:location", payload);
          } else {
            console.warn(`⚠️ Driver location request: no in-memory fallback coordinates for ride ${data.rideId}`);
          }
          return;
        }
        ...
        if (!ride.driver_id || !["accepted", "arriving", "arrived", "in_progress"].includes(ride.status)) {
          const fallback = buildFallbackDriverLocation(data.rideId, ride.driver_id);
          if (fallback) {
            io.to(socket.id).emit("driver:location", fallback);
          } else {
        ...
        let latitude = driverRow?.current_latitude;
        let longitude = driverRow?.current_longitude;
        ...
        if (latitude == null || longitude == null) {
          const storedLocation = latestDriverLocations.get(ride.driver_id);
          if (storedLocation) {
        ...
        if (latitude != null && longitude != null) {
          ...
          const fallback = buildFallbackDriverLocation(ride.id, ride.driver_id);
          if (fallback) {
            io.to(socket.id).emit("driver:location", fallback);
          } else {
            console.warn(`⚠️ Driver location request: no coordinates for driver ${ride.driver_id} on ride ${ride.id}`);
          }
        }
      } catch (err) {
    ...
    socket.on("driver:location", async (location: DriverLocation) => {
      try {
        let actualDriverId = location.driverId;
        const { data: directDriver } = await supabase
          .from("drivers")
          .select("id")
          .eq("id", location.driverId)
      ...
      try {
        ...
        try {
          ...
          if (update.status === "accepted") {
            ...
            if (resolvedDriverId) {
              const storedLocation =
                latestDriverLocations.get(resolvedDriverId) ||
                latestDriverLocations.get(socket.id);

              if (storedLocation) {
                (update as any).driverLocation = {
                  ...storedLocation,
              ...
              try {
                const { data: driverLocationRow } = await supabase
                  .from("drivers")
                  .select("current_latitude, current_longitude")
                  .eq("id", resolvedDriverId)
                  .single();

                if (driverLocationRow?.current_latitude != null && driverLocationRow?.current_longitude != null) {
                  (update as any).driverLocation = {
                    driverId: resolvedDriverId,
                    rideId: update.rideId,
                    latitude: driverLocationRow.current_latitude,
                    longitude: driverLocationRow.current_longitude,
                  };
                }
              } catch (locationErr) {
                console.warn("⚠️ Could not attach driver location on accept:", locationErr);
              }
            }

            if (!(update as any).driverLocation) {
              const fallbackLocation = buildFallbackDriverLocation(update.rideId, resolvedDriverId);
              if (fallbackLocation) {
                (update as any).driverLocation = fallbackLocation;
                console.log(`📍 Using fallback driver location on accept for ride ${update.rideId}:`, fallbackLocation);
              }
            }
          }
          else if (update.status === "arrived") {
