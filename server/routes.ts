...
//   app.get("/api/drivers/user/:userId", async (req: Request, res: Response) => {
//     try {
//       let driver = await storage.getDriverByUserId(req.params.userId as string);
//       if (!driver) {
//         // Auto-create a driver profile if they are trying to access driver features but don't have one yet.
...
//         });
//         if (!driver) {
//           return res.status(404).json({ error: "Driver not found and could not be created" });
//         }
//       }
//       res.json({ driver });
//     } catch (error) {
...
//       if (error && cancelledBy === 'driver') {
//         console.warn("⚠️ later_bookings cancel update failed, retrying without extended driver_cancel fields. Original error:", error.message);
//         delete updatePayload.driver_cancelled_at;
//         delete updatePayload.driver_cancel_reason;
//         delete updatePayload.driver_cancel_type;
//         delete updatePayload.late_cancellation_fee;
//         delete updatePayload.driver_penalty_applied;
